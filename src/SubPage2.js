...
export default class SubPage2 extends Component {
  ...
  imageNotFound(){
    var imageObj = this.props.navigation.state.params.name;
    if(silatimgs[imageObj]){
      defaultImage = silatimgs[imageObj].img
    } else {
      defaultImage = require('../pics/404.png')
    }
    //this.setState({defaultImage : require('../pics/kuda_depan.png')});
        ...
        <Content>
          <Card style={{flex: 1}}>
            <CardItem>
              <Left>
                <Body>
                  <Text style={styles.bigblue}>{ params.name }</Text>
                  <Text note>{ params.trans }</Text>
                </Body>
              </Left>
            </CardItem>
            <CardItem>
